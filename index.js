...
client.on('interactionCreate', async interaction => {
  ...
  const queue = client.distube.getQueue(interaction);
     if (!queue) return interaction.reply(`Müzik çalmıyor`)
     if (isNaN(volume)) return interaction.reply("Bir miktar belirle")
     if (volume < 1) return interaction.reply("Müziğin sesini en az **1** yapabilirsin.")
     if (volume > 250) return interaction.reply("Müziğin sesini en fazla **250** yapabilirsin.")
     client.distube.setVolume(interaction, volume);
     interaction.reply("Müziğin sesini **"+volume+"** olarak ayarladım!")
}
})
client.login(token)
